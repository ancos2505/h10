...
fn main() -> ExitCode {
    match smain() {
        ...
        Err(err) => match err {
            ...
            | ServerError::PoisonErrorRwLockWriteGuard(err)
            | ServerError::PortParseError(err)
            | ServerError::SystemTimeError(err)
