...
use html_rs::{
    Html, HtmlBody,
    elements::{Div, ElementBuilder, Form, H1, Input, Link, Meta, P, TextContent, Title},
};
...
use crate::{
    ROOT_PAGER_COUNTER,
    server::{ServerResponse, ServerResult},
};

pub fn root(request: Request) -> ServerResult<ServerResponse> {
    if let Some(endpoint) = request.query_string.get("endpoint") {
        match &**endpoint.value() {
            "counter" => {
                *ROOT_PAGER_COUNTER.write()? += 1;
    ...
    // TODO:
    let current_counter = ROOT_PAGER_COUNTER.read()?;
